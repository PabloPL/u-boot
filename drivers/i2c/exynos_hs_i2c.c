 ...
 */
static int hsi2c_wait_for_trx(struct s3c24x0_i2c_bus *i2c_bus)
{
	...
	while (i-- > 0) {
		...
		if (i2c_bus->hw_variant == I2C_TYPE_EXYNOS7) {
		...
		} else if (int_status & HSI2C_INT_I2C_EN) {
...
static void hsi2c_ch_init(struct s3c24x0_i2c_bus *i2c_bus)
{
	...
	if (i2c_bus->hw_variant == I2C_TYPE_EXYNOS7) {
		writel(HSI2C_INT_I2C_TRANS, &hsregs->usi_int_en);
	} else {
...
static int hsi2c_write(struct s3c24x0_i2c_bus *i2c_bus,
		       unsigned char chip,
		       unsigned char addr[],
		       unsigned char alen,
		       unsigned char data[],
...
static int hsi2c_read(struct s3c24x0_i2c_bus *i2c_bus,
		      unsigned char chip,
		      unsigned char addr[],
		      unsigned char alen,
		      unsigned char data[],
		      unsigned short len)
{
	...
	if (alen) {
		...
		rv = hsi2c_write(i2c_bus, chip, addr, alen, NULL, 0, false);
		if (rv != I2C_OK)
			return rv;
	}
			      ...
			      int nmsgs)
{
	...
	for (; nmsgs > 0; nmsgs--, msg++) {
		if (msg->flags & I2C_M_RD) {
			ret = hsi2c_read(i2c_bus, msg->addr, 0, 0, msg->buf,
					 msg->len);
		} else {
			ret = hsi2c_write(i2c_bus, msg->addr, 0, 0, msg->buf,
					  msg->len, true);
		}
		if (ret) {
...
static int s3c24x0_i2c_probe(struct udevice *dev, uint chip, uint chip_flags)
{
	 ...
	 */
	ret = hsi2c_read(i2c_bus, chip, 0, 0, buf, 1);
...
static int s3c_i2c_ofdata_to_platdata(struct udevice *dev)
{
	...
	i2c_bus->hw_variant = dev_get_driver_data(dev);
...
static const struct udevice_id exynos_hs_i2c_ids[] = {
	{
		...
		.data = I2C_TYPE_EXYNOS5
	},
	{
		.compatible = "samsung,exynos7-hsi2c",
		.data = I2C_TYPE_EXYNOS7
	},
	{ }
};
